...
class IndexDiscoverViewModel : ViewModel() {
    ...
    val banner = switchMap(discoverBean) {
        "首页刷新图片".toast()
        it?.itemList?.get(0)?.data?.itemList
    }
    val classify = switchMap(discoverBean) {
        it?.itemList?.get(1)?.data?.itemList
    }
    val special = switchMap(discoverBean) {
        it?.itemList?.get(2)?.data?.itemList
    }
    val rankList = switchMap(discoverBean) {
        val list: ArrayList<IndexDiscoverBean.Data>?
        if (it?.itemList?.get(4)?.data == null) {
            return@switchMap null
        }else {
            list = ArrayList(5)
        }
        for (i in 1..5) {
            val data = it.itemList[3 + i].data
            list.add(data)
        }
        list
    }
    val theme = switchMap(discoverBean) {
        val list: ArrayList<IndexDiscoverBean.Data>?
        if (it?.itemList?.get(10)?.data == null) {
            return@switchMap null
        }else {
            list = ArrayList(10)
        }
        for (i in 1..10) {
            val data = it.itemList[9 + i].data
            list.add(data)
        }
        list
    }
